	...
	protected void do_btnAdd_actionPerformed(ActionEvent e) {
		...
		if (ProductValidation.ValidateProductName(cboProduct.getSelectedItem().toString().trim())) //Is the product name valid?
		{
			...
			if (ProductValidation.ValidateQuantity((int)spnQuantity.getValue())) //Is the quantity within the boundaries?
			{
				...
				// if true continue
				if (productInBasket){
					int formatInput = cboFormat.getSelectedIndex() + 1;
					int index = 0;

					// get the index of the product - checks both product name and format
					...
					if (basketItemList.get(index).getFormatID() == formatInput) {
						...
						System.out.println("Adding new product format");
						quantity = (int)spnQuantity.getValue();
